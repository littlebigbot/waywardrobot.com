...
define([
  ...
  var lazyLoadSrc = function( $window, $document ) {
    ...
    var lazyLoader = (function() {
      ...
      function removeScrollContainer(container) {
        for( var i; i < containers.length; i++ ) {
          if( containers[ i ] === container ) {
            var oldContainer = containers.splice( i, 1 );
            $(oldContainer[0]).off('scroll');
            break;
          }
        }
      }
      ...
      function startWatchingWindow() {
        ...
        win.on( "scroll.lazyLoadSrc", windowChanged );
        // @TODO: Figure out if there's a better way to do this:
        $(containers.join(', ')).on('scroll', windowChanged);
      ...
      function stopWatchingWindow() {
        ...
        win.off( "scroll.lazyLoadSrc" );
        $(containers.join(', ')).off('scroll');
      ...
      return({
        ...
        addScrollContainer: addScrollContainer,
    ...
    function link( $scope, element, attributes ) {
      ...
      if(typeof attributes.lazyLoadWatch !== 'undefined') {
        lazyLoader.addScrollContainer( attributes.lazyLoadWatch );
      }
  ...
  return app.directive('lazyLoadSrc', lazyLoadSrc);
});
